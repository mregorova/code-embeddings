...
Если посчитать косинусное расстояние для большего множества задач в рамках языка Python и сравнить (в том числе при помощи графика ниже), можно сделать вывод, что модель не различает решения одинаковых и разных задач :(  
Это важный результат, так как эмбеддинг отражает смысл кода, если модель его действительно понимает.
